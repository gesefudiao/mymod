...
static int show_add_init(void){
    ...
    if(logfile != NULL){
        printk("a\n");
        file_write(logfile, f_offset, (char *)buf, strlen(buf));
        memset(buf, 0, 2048);
    }
	...
	//	printk("error:p_task should be noPointer!\n");
        sprintf(buf, "%s","error:p_task should be noPointer!\n");
		return -EFAULT;
	}
    /*
	printk("*name---------%s\n",p_task->comm);
    ...
    */
    ...
    sprintf(buf+strlen(buf), "*state--------%ld\n\n",p_task->state);
    if(logfile!=NULL){
        printk("c\n");
        file_write(logfile, f_offset, (char *)buf, strlen(buf));
...
//    根据页表扫描进程的物理页面
//    scan_add_tsk(p_task, logfile,buf);
/*    移植到scan_add_task函数中
	p_mm = p_task->mm;
	for (i = 0; i < PTRS_PER_PGD; ++i)
	{
...
*/
    ...
    count_page_tsk(p_task, logfile, f_offset, buf);
    file_close(logfile);
	// return 0;
}
